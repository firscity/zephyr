...
void notify_evtchn(evtchn_port_t port)
{
	...
	if (port >= EVTCHN_2L_NR_CHANNELS) {
		LOG_ERR("%s: trying to send notify for invalid evtchn #%u\n",
				__func__, port);
		return;
	}
...
int bind_event_channel(evtchn_port_t port, evtchn_cb_t cb, void *data) {
	if (port >= EVTCHN_2L_NR_CHANNELS) {
		LOG_ERR("%s: trying to bind invalid evtchn #%u\n",
				__func__, port);
		return -EINVAL;
	}

	if (!cb) {
		LOG_ERR("%s: NULL callback for evtchn #%u\n", __func__, port);
		return -EINVAL;
	}

	if (event_channels[port].cb != empty_callback)
		LOG_ERR("%s: re-bind callback for evtchn #%u\n",
				__func__, port);
...
int unbind_event_channel(evtchn_port_t port) {
	if (port >= EVTCHN_2L_NR_CHANNELS) {
		LOG_ERR("%s: trying to unbind invalid evtchn #%u\n",
				__func__, port);
		return -EINVAL;
	}
...
int mask_event_channel(evtchn_port_t port) {
	...
	if (port >= EVTCHN_2L_NR_CHANNELS) {
		LOG_ERR("%s: trying to mask invalid evtchn #%u\n",
				__func__, port);
		return -EINVAL;
	}
...
int unmask_event_channel(evtchn_port_t port) {
	...
	if (port >= EVTCHN_2L_NR_CHANNELS) {
		LOG_ERR("%s: trying to unmask invalid evtchn #%u\n",
				__func__, port);
		return -EINVAL;
	}
