...
#include <zephyr/xen/xenbus/client.h>
#include <zephyr/xen/xenbus/xs.h>
#include <zephyr/xen/xenbus/xs_watch.h>
...
static void xenbus_read_thrd(void *p1, void *p2, void *p3)
{
	...
	snprintf(buf, 50, "%s", "memory/videoram");
	watch = xs_watch_path(XBT_NIL, buf);

	if (!watch) {
		LOG_ERR("FAILED TO SET WATCH");
		return;
	}
	printk("Watch returned = %p\n", watch);

	bool run = true;
	while (run) {
		int value;

		k_sem_take(&watch->sem, K_FOREVER);
		printk("Watch triggered!\n");

		if (!xs_read_integer(XBT_NIL, buf, &value)) {
			printk("%s == %d\n", buf, value);
		} else {
	...
	int rc = xs_unwatch(XBT_NIL, watch);
	printk("xs_unwatch rc == %d\n", rc);

	snprintf(buf, 50, "%s", "/local/domain/1/memory/static-max");
	watch = xs_watch_path(XBT_NIL, buf);
